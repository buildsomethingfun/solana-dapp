...
import dapp.buildsomething.repository.solana.internal.core.Constants.PublicKeyLength
import dapp.buildsomething.repository.solana.internal.core.Constants.SignatureLength
import dapp.buildsomething.repository.solana.internal.core.instruction.BaseInstruction
import dapp.buildsomething.repository.solana.internal.core.instruction.Instruction
...
class Transaction(
    ...
    private fun transactionMessage(): ByteArray {
        ...
        val bufferSize =
            HEADER_LENGTH +
                ...
                (accountKeys.size * PublicKeyLength) +
    ...
    fun serialize(): ByteArray {
        ...
        val buffer =
            ByteBuffer.allocate(
                signaturesLength.size + signatures.size * SignatureLength + message.size,
            )
    ...
    companion object {
        ...
        fun from(encodedTransaction: String): Transaction {
            ...
            for (i in 0 until signaturesDecodedLength.length) {
                ...
                val zeroSignature = Base58.encode(ByteArray(SignatureLength))
            ...
            for (i in 0 until accountDecodedLength.length) {
                val account = byteArray.slice(0 until PublicKeyLength)
                byteArray = byteArray.drop(PublicKeyLength).toByteArray()
                accountKeys.add(Base58.encode(account.toByteArray()))
            }

            val recentBlockhash = byteArray.slice(0 until PublicKeyLength).toByteArray()
            byteArray = byteArray.drop(PublicKeyLength).toByteArray()
