...
import dapp.buildsomething.repository.solana.internal.core.instruction.CompiledInstruction
    ...
    companion object {
        ...
        fun deserialize(d: ByteArray): TransactionMessage {
            ...
            for (i in 0 until accountKeyDecodedLength.length) {
                val account = data.slice(0 until PublicKeyLength)
                data = data.drop(PublicKeyLength).toByteArray()
                accountKeys.add(PublicKey(account.toByteArray()))
            }

            val recentBlockhash = data.slice(0 until PublicKeyLength).toByteArray().also {
                data = data.drop(PublicKeyLength).toByteArray()
            }
            ...
            if (version == MessageVersion.V0) {
                ...
                for (i in 0 until addressLookupTableDecodedLength.length) {
                    val account = data.slice(0 until PublicKeyLength).toByteArray().also {
                        data = data.drop(PublicKeyLength).toByteArray()
                    }
